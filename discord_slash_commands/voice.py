...
async def voice_join(ctx):
    ...
    ctx.bot.add_cog(AudioQueue(ctx.bot.voice_clients[0]))
    await ctx.respond(
        ephemeral = False,
        delete_after = 60*30,
...
voice_queue_slash_command_group = discord.SlashCommandGroup(
    checks = [
        ctx_check.assert_bot_is_in_voice_chat,
        ctx_check.assert_bot_is_in_same_voice_chat_as_author,
    ...
    #name_localizations = default,
    #nsfw = default,
    parent = voice_slash_command_group
)
...
async def voice_queue_remove(
    ...
    audio_queue = bot.ctx.cogs.get_cog("AudioQueue")
    if audio_queue_element_id is None:
        audio_queue_element_id = audio_queue.queue[0].audio_queue_element_id

    # Try to remove audio_queue_element_id from audio_queue
    if audio_queue.remove(audio_queue_element_id) is False:
        await ctx.respond(
            ephemeral=True,
...
async def voice_queue_list(ctx):
    ...
    for audio_queue_element in audio_queue.queue:
